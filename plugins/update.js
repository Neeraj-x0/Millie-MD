...
let handler = async (m, {usedPrefix,command, conn, text, isROwner }) => {
  try{if (global.conn.user.jid == conn.user.jid) {
    let stdout = execSync('git init && git remote set-url origin https://github.com/Neeraj-x0/Millie-MD.git && git pull' + (isROwner && text ? ' ' + text : ''))
