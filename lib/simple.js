    ...
    */
    ...
    conn.sendTBD = async (jid, contentText, footer, thumb, dtux, urlx, dtcx, nmbrx, buttons1, row1, buttons2, row2, buttons3, row3, quoted, options) => {
        const message = {
            ...
            jpegThumbnail: await (await fetch(thumb)).buffer(), fileName: global.nd, mimetype: global.td,
            caption: contentText,
            footer: footer,
            templateButtons: [
                {
...
/**
  ...
  */
    ...
    conn.sendTBVG = async (jid, contentText, footer, video, dtux, urlx, buttons1, row1, buttons2, row2,quoted, options) => {
        const message = {
            video: { url: video }, ...options,
            gifPlayback: true, jpegThumbnail: await (await fetch(thumbt)).buffer(),
            caption: contentText,
            footer: footer,
            templateButtons: [
                {
...
exports.smsg = (conn, m, hasParent) => {
    ...
    /*m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'ꜱɪᴍᴩʟᴇ ᴡʜᴀᴛꜱᴀᴩᴩ ʙᴏᴛ', body: 'Neeraj-x0', sourceUrl: 'https://wa.me/918113921898', thumbnail: data }}, options })
    }
    /**
     * Copy this message
     */
